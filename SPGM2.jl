...
@with_kw mutable struct SPGM2 <: FOM
    memory_size
    max_steps
    x0
    x
...
SPGM2(memory_size) = SPGM2(memory_size,missing,missing,missing,missing,missing,
                           missing,missing,missing,missing,missing,missing,
                           missing,missing,missing)
SPGM2(memory_size, max_steps) = SPGM2(memory_size,max_steps,missing,missing,
                                    missing,missing,missing,missing,missing,
                                    missing,missing,missing,missing,missing)

function initialize(method::SPGM2, start::Vector{Float64})
    method.x0 = start
...
function update(method::SPGM2, value::Float64, gradient::Vector{Float64};
        tol=1e-9)
    @unpack_SPGM2 method
...
function optimize_lambdas(taus, Z, G, h, q, v, vmin; tol=1e-8)
    ...
    # Set all of the entries of mu equal to zero except first and last.
    if k <= 2
        y = nonnegSecondOrderCone(hcat(Z, -G),
                                  vcat(mucoeffs, lambdascoeffs),
                                  vcat(taus, ones(k)), tol=tol)
        mu = y[1:k]
        lambdas = y[k+1:end]
        return mu, lambdas
    end
    y = nonnegSecondOrderCone(hcat(Z[:,1], Z[:,end], -G),
                              vcat(mucoeffs[1], mucoeffs[end], lambdascoeffs),
                              vcat(taus[1], taus[end], ones(k)), tol=tol)
    mu = vcat(y[1], zeros(k-2), y[2])
    lambdas = y[3:end]
...
function guarantee(method::SPGM2)
    # todo fix this
    return 1/method.taus[end]
end

function methodTitle(method::SPGM2)
